...
if(empty($inputData['author']))
{
  ...
  if($for=="add")
  {
    ...
    {
    $_SESSION['errors']['book']='Book is required';
    }
  }
...
if (isset($_POST['add'])) {
  ...
  if($error==false)
  {
    ...
    if (!empty($_FILES["image"]["name"]) && !empty($_FILES["book"]["name"]))
    {
        ...
        if (in_array($fileType, $allowedTypes)) {
          ...
          if(move_uploaded_file($_FILES["book"]["tmp_name"], $targetBookPath))
          {
            if (move_uploaded_file($_FILES["image"]["tmp_name"], $targetFilePath))
            {
              ...
              $sql = "INSERT INTO books (bookName,description,image,userID,publishedDate,price,genreID,premium,book,author) VALUES ('".$bookName."', '".$desc."','".$fileName."','".$postBy."',CURRENT_TIMESTAMP,'".$price."','".$genre."','".$premium."','".$bookFileName."','".$author."')";
      ...
      else
      {
        $response = [
          'status' => 'error',
          'message' => "Invalid book format.<br> Only PDF files are allowed.",
        ];
        $_SESSION['alert'] = $response;
        $_SESSION['old_data']=$inputData;
        echo 
...
if (isset($_POST['update'])) {
  ...
  if($error==false)
  {
    ...
    $qry = "update books set bookName = '$bookName',description='$desc',genreID='$genre',premium='$premium',price='$price',author='$author'";
